		...
		*** method 2
		...
		foreach p of global phenolst {
			...
			* output rs and ra and es as text file
			outfile using "prx_es_ra_`p'.raw", noquote replace
			outfile rs using "prx_`p'.raw", noquote replace
			* plink to extract the snps
			*!plink2 --bfile D:\data_work/`x'\geno/`y' --extract prx_`p'.raw --make-bed --out D:\data_work/`x'\geno/prx_`p'
			* plink to calculate GRS with --score (rs ra es)
			*!plink2 --bfile D:\data_work/`x'\geno/prx_`p' --score prx_es_ra_`p'.raw --out D:\data_work/`x'\geno/grs_`p' // this will give a different genotyping rate in the log file
			!plink2 --bfile D:\data_work/`x'\geno/`y' --score prx_es_ra_`p'.raw --out D:\data_work/`x'\geno/grs_`p'
